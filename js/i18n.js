...
window.I18n = (function () {
  ...
  var translations = {
    ...
    ja: {
      ...
      'contact.faq.q2': '「ホーム画面に追加」のやり方がわからない',
      'contact.faq.a2': '<p>お使いの端末やブラウザによって操作が異なります。</p><ul><li><strong>iPhone / iPad</strong>：<strong>Safari</strong> でゲームのページを開き、画面下の共有ボタン（□に↑のマーク）をタップ →「ホーム画面に追加」を選択してください。※ Chrome等のブラウザでは追加できません。</li><li><strong>Android</strong>：<strong>Chrome</strong> でゲームのページを開き、右上のメニュー（︙）をタップ →「ホーム画面に追加」または「アプリをインストール」を選択してください。</li><li><strong>PC</strong>：Chrome / Edge でゲームのページを開き、アドレスバー右端のインストールボタン（⊕）をクリック。表示されない場合はメニュー（︙）→「アプリをインストール」からも追加できます。</li></ul><p>詳しい手順は<a href="index.html#pwa-guide">スマホにアプリとして追加する方法</a>をご覧ください。</p>',
      'contact.faq.q3': 'ゲームの表示が画面からはみ出る・崩れる',
      'contact.faq.a3': '<p>一度ホーム画面からアプリを削除して、もう一度追加し直すと改善する場合があります。また、端末のOSやブラウザのバージョンが古いと正しく表示されないことがあります。</p><ul><li>端末のOSを最新バージョンにアップデートしてください</li><li>ブラウザ（Safari / Chrome）も最新バージョンに更新してください</li></ul><p>それでも解決しない場合は、お問い合わせフォームから端末名とブラウザ名をお知らせください。</p>',
      'contact.faq.q4': 'ホーム画面に追加したゲームはオフラインでも遊べますか？',
      'contact.faq.a4': '<p>はい、一度ホーム画面に追加すればインターネットに繋がっていなくても遊べます。ただし、最初にページを開くときやアップデートがあるときはインターネット接続が必要です。</p>',
    ...
    en: {
      ...
      'contact.faq.q2': 'I don\'t know how to add a game to my home screen',
      'contact.faq.a2': '<p>The steps differ depending on your device and browser:</p><ul><li><strong>iPhone / iPad</strong>: Open the game page in <strong>Safari</strong>, tap the Share button (square with upward arrow) at the bottom, then select "Add to Home Screen." * This does not work in Chrome or other browsers.</li><li><strong>Android</strong>: Open the game page in <strong>Chrome</strong>, tap the menu (⋮) in the top right, then select "Add to Home screen" or "Install app."</li><li><strong>PC</strong>: Open the game page in Chrome / Edge, click the install button (⊕) at the right end of the address bar. If it doesn\'t appear, go to menu (⋮) → "Install app."</li></ul><p>For detailed steps, see <a href="index.html#pwa-guide">How to add games to your phone</a>.</p>',
      'contact.faq.q3': 'Game display is broken or overflows the screen',
      'contact.faq.a3': '<p>Removing the app from your home screen and adding it again may fix the issue. Display problems can also occur if your device OS or browser is outdated.</p><ul><li>Update your device OS to the latest version</li><li>Update your browser (Safari / Chrome) to the latest version</li></ul><p>If the problem persists, please contact us with your device model and browser name.</p>',
      'contact.faq.q4': 'Can I play games added to my home screen offline?',
      'contact.faq.a4': '<p>Yes, once added to your home screen, you can play without an internet connection. However, you\'ll need internet access when opening the game for the first time or when an update is available.</p>',
